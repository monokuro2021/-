  ...
  card.onclick = () => {
    ...
    if (!firstCard) {
      ...
      if (firstCardNumber === secondCardNumber) {
        ...
        if (cardNum >= 1 && cardNum <= 10 && !isLastPair) {
          showGetMessage(cardNum);
        }
        
        // ゲットメッセージ表示中はカード除去を待つ（最後のペアの場合は即時実行）
        const delay = isLastPair ? 0 : 2000;
        setTimeout(() => {
          // すぐにカードを盤面から除去
          ...
          if (remainingCards.length === 0) {
            ...
            setTimeout(() => {
              ...
              if (player1Score > player2Score) {
                showGeneralMessage(`<span style="color: ${player1Color}; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5);">${player1CharacterName}</span> <span style="text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5);">勝利！</span>`, 0);
              } else if (player2Score > player1Score) {
                showGeneralMessage(`<span style="color: ${player2Color}; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5);">${player2CharacterName}</span> <span style="text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5);">勝利！</span>`, 0);
              } else {
                showGeneralMessage("引き分け！", 0);
              }
...
function useSkill(player, skillType) {
  if (lock) { showGeneralMessage('カード処理中は発動できません！'); return; }
  
  if (player !== currentPlayer) { showGeneralMessage('自分の手番でないと発動できません！'); return; }
  
  if (firstCard) { showGeneralMessage('カード選択中は発動できません！'); return; }
  ...
  // 既に待機中のスキルがあるかチェック
  if (pendingSkills[player] !== null) { showGeneralMessage('すでに他のスキルが待機中です！'); return; }
...
function handleAfterTwoCards() {
  ...
  if (pendingBlock) {
    ...
    showGeneralMessage('カードを１枚選択してください！');
  } else {
